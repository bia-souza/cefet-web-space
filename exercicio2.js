...
const goTo = (img) => {
  ...
  image.src = servidorDasImagens + '/' + img.arquivo;
  image.alt = img.descricao;
}

const previousImage = () => {
  const index_atualizado = (index - 1) % imagens.length;
  if (index_atualizado >= 0){
	  index = index_atualizado;
  }
  else {
	  index_atualizado + imagens.length;
  }
  goTo(imagens[index]);
}
